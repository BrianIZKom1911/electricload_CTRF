# Region: NC
# Continue with 'S1_0_prepare_allregions.R' and regional running data.
# This script is meant to generate the exact results used in my paper.
# So, this file merely serves for reproduction and verification of the results.
rm(list = ls())
...
get_BPT <- function(TRF, order, t_range){
    ...
    trf_s <- function(s){
        ...
        x2 <- c(
            sin(2*pi*s), cos(2*pi*s), sin(4*pi*s), cos(4*pi*s), 
    ...
    r_o <- s_o*(b-a)+a
    y_min <- ans$objective
    list_min <- list(s_opt = s_o, BPT = r_o, BPE = y_min)
    return(list_min)
}
...
ctrf_12 <- lm(
    log(load) ~ ts+sin1+cos1+sin2+cos2
    +ntd+ntd:ts+ntd:sin1+ntd:cos1
    +prcp+prcp:ts+prcp:sin1+prcp:cos1
    +rhum+rhum:ts+rhum:sin1+rhum:cos1
    +wsp+wsp:ts+wsp:sin1+wsp:cos1
    +skc+skc:ts+skc:sin1+skc:cos1
    +m01+m02+m03+m04+m05+m06+m07+m08+m09+m10+m11, data=dt_hr
)
